...
def fac : ℕ → ℕ
  | 0 => 1 -- therse arrows are just = >
  | n + 1 => (n + 1) * fac n

lemma fac_zero : fac 0 = 1 := by
{
  rfl
}
...
example (n : ℤ) (h : (n : ℚ) = 3) : 3 = n := by norm_cast at h ; rw [h]

example (q q' : ℚ) (h : q ≤ q') : exp q ≤ exp q' := by gcongr ; norm_cast

example (n : ℤ) (h : 0 < n) : 0 < sqrt n := by rw [sqrt_pos] ; norm_cast
